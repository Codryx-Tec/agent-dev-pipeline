...
// SCOPE-0.6.0.md §2.4 — the RFC's executable structure. Opt-in: a native
// decision enters this stricter shape only when it declares
// `**Decision criteria:**` or `**Options considered**` itself — every
// decision that predates this (this repo's own 16, `.exemplo/`'s 2) stays
// exactly as it was, unaffected. See rfc.js's module header for why
// retrofitting a scoring matrix onto an already-settled decision would be
// the antipattern this family exists to catch, not compliance with it.
const RE_CRITERIA_MARKER = /\*\*Decision criteria:\*\*\s*(.+)$/m;
const RE_SCORED_OPTIONS_MARKER = /^\*\*Options considered\*\*\s*$/m;
...
// at the first line break. The trailing whitespace here is deliberately
// same-line only ([ \t], not \s) — a \s* here would eat the blank line that
// marks where the justification ends, before that search even runs.
...
function parseTableRows(text) {
  return [...text.matchAll(RE_TABLE_ROW_RAW)]
    .map((m) => m[1].split('|').map((c) => c.trim()))
    .filter((cells) => !cells.every((c) => RE_TABLE_SEPARATOR.test(c)));
}

/**
 * A decision's §2.4 structure, or `null` when it never opted in. Returns
 * enough to both drive the new G2 checks AND override the plain-native
 * `alternatives`/`hasDoNothing` computation, since a scored decision uses
 * `OPT-xxx` bullets, not the numbered `1. *Name.*` list the older shape reads.
 */
function parseScoredStructure(body) {
  const criteriaAt = body.search(RE_CRITERIA_MARKER);
  const optionsAt = body.search(RE_SCORED_OPTIONS_MARKER);
  if (criteriaAt === -1 && optionsAt === -1) return null;

  const criteriaIds = criteriaAt === -1 ? [] : (body.match(RE_CRITERIA_MARKER)[1].match(/W-\d+/g) ?? []);

  // Options run from the marker to the Scoring matrix marker, or to the end.
  const matrixAt = body.search(RE_SCORING_MATRIX_MARKER);
  const optionsRegion =
    optionsAt === -1 ? '' : body.slice(optionsAt, matrixAt === -1 ? body.length : matrixAt);
  const options = [...optionsRegion.matchAll(RE_OPT_ITEM)].map((m) => ({
    id: m[1],
    name: m[2].trim(),
    requires: splitRequires(m[3].match(RE_REQUIRES)?.[1] ?? ''),
    // The option's own prose, past its bold name — CONTEXT_NUMBER_WITHOUT_SOURCE
    // (audit.js) reads this for an uncited numeric claim.
    prose: m[3].trim(),
  }));

  // The matrix table runs from its marker to `**Recommendation:**` or the
  // decision's own end.
  const recommendationAt = body.search(RE_RECOMMENDATION);
  const matrixRegion =
    matrixAt === -1
      ? ''
      : body.slice(matrixAt, recommendationAt === -1 ? body.length : recommendationAt);
  const rows = parseTableRows(matrixRegion);
  ...
  if (header) {
    for (const row of dataRows) {
      ...
      if (!optId) continue;
      const cells = {};
      for (let i = 1; i < header.length; i++) cells[header[i]] = row[i] ?? null;
      matrix[optId] = cells;
    }
  }

  const recMatch = body.match(RE_RECOMMENDATION);
  // The justification runs to the next blank line or the next `**Marker:**`
  // line (typically `**Decision:**`), whichever comes first — a wrapped
  // paragraph, not just its opening line.
  const recommendation = recMatch
    ? (() => {
        const rest = body.slice(recMatch.index + recMatch[0].length);
        const blankAt = rest.search(/\n[ \t]*\n/);
        const nextMarkerAt = rest.slice(1).search(RE_ANY_BOLD_MARKER);
        const stopAt = [blankAt, nextMarkerAt === -1 ? -1 : nextMarkerAt + 1]
          .filter((i) => i !== -1)
          .sort((a, b) => a - b)[0];
        const text = stopAt === undefined ? rest : rest.slice(0, stopAt);
        return { optId: recMatch[1], justification: text.replace(/\s+/g, ' ').trim() };
      })()
    : null;

  return {
    criteriaIds,
    // Order is normative (§2.4): criteria must be declared before options.
    // Missing entirely while a matrix exists is the same failure — nothing
    // to be "before" anything.
    criteriaBeforeOptions: criteriaAt !== -1 && (optionsAt === -1 || criteriaAt < optionsAt),
    criteriaDeclared: criteriaAt !== -1,
    options,
    matrixHeader: header ?? [],
    matrix,
    recommendation,
...
function parseNativeDecisions(scan, content, file) {
  return blocksBetween(scan, [...scan.matchAll(RE_DECISION)]).map(({ match, body }) => {
    ...
    const scored = parseScoredStructure(body);
    ...
    return {
      ...
      // A scored decision's options are OPT-xxx bullets, not the plain
      // numbered list — count and "do nothing" both read from there instead
      // once a decision has opted in.
      alternatives: scored ? scored.options.length : (altRegion.match(RE_NUMBERED) || []).length,
      decided: RE_DECIDED.test(body),
      hasDoNothing: scored
        ? scored.options.some((o) => o.id === 'OPT-000' && RE_DO_NOTHING_NAME.test(o.name))
        : altNames.some((n) => RE_DO_NOTHING_NAME.test(n)),
