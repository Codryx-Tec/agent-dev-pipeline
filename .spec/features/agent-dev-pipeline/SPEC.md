...
## T-071 — `OPTION_DO_NOTHING_MISSING` retrofitted onto this repo's own 16 pre-existing decisions [done]

- Refs: AC-092
- Files: .spec/rfc/RFC-001-agent-dev-pipeline.md, .spec/BACKLOG.md
- Notes: `T-020`'s own note recorded the decision to ship `OPTION_DO_NOTHING_MISSING` as a warning specifically so it would never retroactively break this repo's own already-settled RFC, and `.spec/BACKLOG.md` carried the 16 resulting warnings as accepted debt on the stated reasoning that retrofitting them "would be ceremony for its own sake." On explicit instruction, that judgment call is reversed here — not because the original reasoning was wrong, but because the person who made it is the one now asking for it to be revisited.

  Each of `D-001` through `D-016` gained a genuine, specific "do nothing" alternative — what NOT deciding would actually have meant for that particular choice, not a placeholder. Five (`D-002`, `D-008`, `D-011`, `D-013`, `D-016`) already had an alternative that was do-nothing in substance without the name — "no background execution," "excluded entirely," "remove it," "keep it dead," "keep the Portuguese vocabulary" — and were renamed in place to say so honestly, rather than padding the list with a redundant near-duplicate; renaming an already-rejected alternative's own name also carries zero renumbering risk, since `**Decision: alternative N**` references a position, never a name. The other eleven gained a new, final alternative appended after the existing ones, for the same reason: appending never shifts an earlier alternative's number, and every `**Decision: alternative N**` line in this file names a number, not a description.

  Verified for real: `adp audit --ci` against this repo's own `.spec/` now reports zero `OPTION_DO_NOTHING_MISSING` findings, confirmed by grep against the actual command's own output, not inferred from the diff. One real, non-obvious side effect worth recording rather than being surprised by later: with the 16 warnings gone, `G2` itself now reports `n/a` instead of `clean (16 warning(s))` — not a regression, `gates.js`'s own "n/a never suppresses a real finding" rule (`AC-058`) was masking G2's true ceremony state the entire time these findings existed, since this feature's own `> signals:` (`hard-to-reverse` alone) never actually reaches `rfc-first` ceremony. G2 was never really due here; it just had something real to say.
