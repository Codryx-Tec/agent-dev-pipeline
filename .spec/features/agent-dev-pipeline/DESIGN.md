...
**§2.3, the conditional RFC** (`Door:` on `Q-xxx`). Part A of the largest `BACKLOG.md` item; `RFC_REQUIRED`/`DOOR_UNDECLARED` read the same way `status:`/`**blocking**` already do — one inline marker. `DOOR_UNDECLARED` is unconditional, which meant retrofitting this repo's own 10 `Q-xxx` and `.exemplo/`'s 1 for real. `RFC_REQUIRED` reads `status: answered` as sufficient closure rather than mining prose for a cited RFC id. §2.4 (weighted criteria, scoring, `OPTION_BEYOND_TEAM`) is Part B, separate.
