...
**Wiring the self-audit into CI** (M7, PRD-006). `adp verify` then `adp audit --ci` against this repository's own `.spec/`, on every push — 0.6.0's own readiness criterion. `.spec/verification/*.json` is now gitignored, same reasoning `.exemplo/`'s copy already had: proof committed before a commit reads `PROOF_STALE` on the very next checkout — caught by actually cloning the repo fresh. That run surfaced two real bugs: `ADP_TRUST_TEST_COMMAND=1` (CI's own approval) leaked into the spawned suite and made `test/plan.test.js`'s AC-048 read an unapproved command as trusted, fixed by isolating that test's consent environment explicitly; and bumping `VERSION` to `0.6.0` (`package.json`, `CHANGELOG.md`'s `[Unreleased]` closed) exposed `AC-056`'s migration-count test as correct only by coincidence — from `0.4.0`, the current `VERSION` chains both registered migrations, not one, verified by actually running them before rewriting the assertion.
