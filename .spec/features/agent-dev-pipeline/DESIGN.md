...
the no-git mtime fallback all followed later — see their own paragraphs below.
...
**Declared deferral** (`parsers/deferrals.js`, `core/audit.js`, M5b).
...
wrong answer — this package leaves nothing outside the project. `init`
writes `./adp`/`adp.cmd` instead, unconditionally, pinned to the
installing `VERSION`, through the same `writeIfMissing` every other file
uses. `--shell-alias` stays opt-in, behind the same "type yes"
confirmation `adp trust` already asks before touching something outside
the repository; PowerShell profile editing is deferred, unverifiable
...
**Documentation rewrite** (M6b, PRD-007). `README.md`/`ARCHITECTURE.md`/`INSTALL.md`/`CHANGELOG.md`/`payload/AGENTS.md`/the `adp` skill, regraded against what M3–M6 shipped. `README.pt-BR.md` was regenerated from the finished English file, not patched. `test/docs.test.js` (AC-117) greps each file for those fossils.
**The two worked examples** (M6b, PRD-007). `.exemplo/` gained `BACKLOG.md`, a real `adp profile`/`estimate`/`close` cycle (`metrics.historyPath` kept local), and a "do nothing" alternative on both RFC decisions. `.exemplo-legado/` is new: a small, real legacy project shipped raw (no `.spec/`, no `.git/`); `START-HERE.md` walks `git init` → `init --brownfield` → the ratchet, every line run for real first. `§2.3`/`§2.4` were found unbuilt, recorded in `BACKLOG.md`. `test/examples.test.js` guards both.

**Wiring the self-audit into CI** (M7, PRD-006). `adp verify` then `adp audit --ci` against this repository's own `.spec/`, every push. `.spec/verification/*.json` is now gitignored, same reasoning `.exemplo/`'s copy already had. That run surfaced two real bugs: `ADP_TRUST_TEST_COMMAND=1` leaking into the spawned suite made a trust test pass for the wrong reason, fixed by isolating its consent environment explicitly; bumping `VERSION` to `0.6.0` exposed a migration-count test as correct only by coincidence, fixed against what actually running both migrations produces.
**§2.3, the conditional RFC** (`Door:` on `Q-xxx`). Part A of the largest `BACKLOG.md` item; `RFC_REQUIRED`/`DOOR_UNDECLARED` read the same way `status:`/`**blocking**` already do — one inline marker. `DOOR_UNDECLARED` is unconditional, which meant retrofitting this repo's own 10 `Q-xxx` and `.exemplo/`'s 1 for real. `RFC_REQUIRED` reads `status: answered` as sufficient closure rather than mining prose for a cited RFC id.
**§2.4, the RFC's executable structure** (`SCOPE.md §11`, `parsers/rfc.js`, `core/ceremony.js`). Part B, opt-in: a decision reaches `CRITERIA_AFTER_OPTIONS`/`RECOMMENDATION_AGAINST_SCORE`/`CONTEXT_NUMBER_WITHOUT_SOURCE`/`OPTION_BEYOND_TEAM` only by declaring `**Decision criteria:**` or `**Options considered**` itself — none of this repo's own 16 decisions or `.exemplo/`'s 2 are touched. A missing `Total` column falls back to summing an option's own criteria scores. `OPTION_BEYOND_TEAM` and the `new-tech` auto-light share one function, `computeCeremonySignals()`, called from both `auditProject` and the CLI's own gate-evaluation path, so a gate's applicability can never quietly disagree with the finding that drove it. `capabilityGapMultiplier` (default `1.5`) is a flat, informational guess — same deferral PRD-003c-history-core already recorded, not a new one.
**Six more agent harnesses, and a config escape hatch for any other one** (`core/paths.js`, `core/install-map.js`). `AGENT_SKILL_DIRS` gained `windsurf`/`gemini`/`copilot`/`cline`/`opencode`/`kilocode`, each verified against that tool's own current docs rather than copied from either reference project. Copilot's auto-detect probes its full `.github/skills` path, not the bare `.github/` every other repo already has for unrelated reasons; adding `antigravity` to the probe list (it was in the map but never the candidate array) makes it correctly ambiguous with `codex` instead of silently mislabeled, no file placement changes. `agent.skillsDir` in `adp.config.json` mirrors `agent.command`'s own escape hatch for invocation: an explicit override installs skills for any harness, known or not, read live from config on every `init`/`upgrade`, never cached into the lockfile. `cli.js` also now asks which agent to use, but only when `detectAgent()` (called once, before `initProject()`) genuinely can't say — nothing found, or more than one candidate — and only in a real terminal; `--yes` or a non-TTY keep today's silent default exactly as before. The decision and the answer parsing (`shouldPromptForAgent`/`resolveAgentAnswer`) are pure functions in `init.js`, not `cli.js`, specifically so they carry a real test — only the list rendering and the one line of input stay in `cli.js`'s own untested-by-convention I/O layer; `initProject()` stays pure and unprompted.
**A declared, non-enforced documentation language** (`SCOPE.md`'s `**Docs language:**`, `project.js`). D-016 fixed the engine's own token vocabulary in English, one spelling, forever — but `AGENTS.md` had separately hardcoded all generated PROSE to English too, with no way to opt out, even though it installs into every project regardless of the team's own language. The new field is declarative only, like `**Decision:**`, free text (not a fixed enum — a language name is never a value the engine compares), defaulting to English when absent so no existing project's behavior changes. Not surfaced in `adp report` yet — nothing consumes it beyond the agent's own instructions. `spec.js`'s `SECTIONS`/`CLAUSES` bilingual acceptance (never part of D-016's five token families — a heading is a landmark, never a compared value) is now a formalized decision, not a leftover; see the comment above `CLAUSES`.
**Brownfield archiving, `adp archive`** (`core/archive.js`, D-017). PRD-002's "Passo 3", the one operation in the whole tool that moves a user's real files, scored per `SCOPE-0.6.0.md §11`'s own instruction that this decision be a scored RFC — which surfaced `SCOPE.md` had never actually grown a `## 11. Decision criteria` section despite §2.4 existing since T-063; its six `W-xxx` carried over from `SCOPE-0.6.0.md §4`'s own already-confirmed weights rather than invented. Copies by default; `--move` opts into `git mv`. Three guards, no override on either flag: not a git repository, a dirty working tree, or (regardless of mode) an untouchable basename (`README.md`/`LICENSE`/`CONTRIBUTING.md`/`SECURITY.md`/`CODE_OF_CONDUCT.md`, matched loosely since `RECOGNITION_GLOBS`'s own `docs/**` can reach a non-root one) or a path a CI workflow's raw text literally references (a documented, not solved, limitation — no YAML parsing). Mirrors `upgrade.js`'s pure plan/apply split; `applyArchive()` re-checks both guards itself, defense against the time gap the typed-`yes` confirmation opens.
...
**`adp close` records exercised capabilities, and a real parser bug the work found** (`closure.js`'s `capabilitiesExercised()`, `rfc.js`'s `scored.decidedOptionId`). Data-collection half of §2.4's capability-multiplier deferral — reads `.spec/rfc/*.md` directly (not `loadProject()`), collects `Requires:` tags from each DECIDED scored decision's chosen option only. Needed knowing which `OPT-xxx` was actually decided, which surfaced a real bug testing it against this repo's own `D-017`: `RE_OPT_ITEM`'s `.+?` never matched a title wrapping onto a second line, silently dropping `OPT-003` from `scored.options` (present in the matrix by direct lookup, absent from gap-checks that walk the options list). Fixed with `[\s\S]+?`.
**`OPTION_DO_NOTHING_MISSING` retrofitted onto this repo's own 16 decisions** (`D-001`–`D-016`). `T-020` shipped the finding as a warning specifically so it would never retroactively break this repo's own already-settled RFC, and `BACKLOG.md` carried the resulting 16 warnings as accepted debt on the stated reasoning that fixing them would be ceremony for its own sake; on explicit instruction that judgment call is reversed — the person who made it is the one now asking it be revisited. Five alternatives that were already do-nothing in substance ("no background execution," "excluded entirely," "remove it," "keep it dead," "keep the Portuguese vocabulary") were renamed in place to say so; the other eleven gained a new, final alternative appended after the existing ones. Both techniques carry zero renumbering risk, since `**Decision: alternative N**` always names a position, never a description. One side effect worth recording, not a regression: with all 16 warnings gone, `G2` itself now reports `n/a` instead of `clean` — `AC-058`'s "n/a never suppresses a real finding" rule had been masking G2's true ceremony state the whole time, since this feature's own `PRD.md` signals (`hard-to-reverse` alone) never reach `rfc-first`.
