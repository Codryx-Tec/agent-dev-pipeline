...
**Documentation rewrite** (M6b, PRD-007). `README.md`/`ARCHITECTURE.md`/`INSTALL.md`/`CHANGELOG.md`/`payload/AGENTS.md`/the `adp` skill, regraded against what M3–M6 shipped. `README.pt-BR.md` was regenerated from the finished English file, not patched — patching produced the self-contradicting document PRD-007 itself gives as its cautionary example. `test/docs.test.js` (AC-117) greps each file for those fossils.

**The two worked examples** (M6b, PRD-007). `.exemplo/` gained `BACKLOG.md`, a real `adp profile`/`estimate`/`close` cycle (`metrics.historyPath` kept local, so `adp close` never touches a real machine's own state), and a "do nothing" alternative on both RFC decisions. `.exemplo-legado/` is new: a small, real legacy project shipped raw (no `.spec/`, no `.git/`); `START-HERE.md` walks `git init` → `init --brownfield` → the ratchet, every line of output run for real first. `§2.3`/`§2.4` (`Door:`, weighted criteria) were found unbuilt, recorded in `BACKLOG.md`. `test/examples.test.js` (AC-118/AC-119) guards both.

**Wiring the self-audit into CI** (M7, PRD-006). `adp verify` then `adp audit --ci` against this repository's own `.spec/`, on every push — the step PRD-006 named as 0.6.0's own readiness criterion. `.spec/verification/*.json` is now gitignored, the same reasoning `.exemplo/`'s copy already had: proof committed before a commit is proof of the parent, and reads `PROOF_STALE` on the very next checkout — caught by actually cloning the repo fresh, not by inspecting the workflow file. That same run surfaced a real, 100%-reproducible bug: `ADP_TRUST_TEST_COMMAND=1` (CI's own outer approval) leaked into the spawned test suite and made `test/plan.test.js`'s AC-048 read an unapproved command as trusted — latent since that variable existed, never triggered because nothing had run this repo's own trust tests under it before. Fixed by isolating that one test's consent environment explicitly rather than inheriting `process.env`.
